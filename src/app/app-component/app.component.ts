...
export class AppComponent implements OnInit  {
  ...
  searchApt(theQuery: string) {
    this.modifiedList = this.theList.filter(eachItem => {
      return(
        eachItem['petName'].toLowerCase().includes(theQuery.toLocaleLowerCase()) ||
        eachItem['ownerName'].toLowerCase().includes(theQuery.toLocaleLowerCase()) ||
        eachItem['aptNotes'].toLowerCase().includes(theQuery.toLocaleLowerCase())
      );
    });
    this.sortItems();
  }
